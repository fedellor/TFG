...
Descripción: Genera un gráfico dual para comparar rendimiento clásico y cuántico (VQE).
"""
import json
import os
...
def generar_grafico_vqe():
    ...
    c_ideal = datos.get("counts_vqe_ideal", {e: 0 for e in estados_ordenados})
    c_ruido = datos.get("counts_vqe_ruido", {e: 0 for e in estados_ordenados})
    ...
    ax1.bar(x - width/2, prob_ideal, width, label='VQE Ideal (Aproximación Teórica)', color='#4c72b0', alpha=0.9)
    ax1.bar(x + width/2, prob_ruido, width, label='VQE NISQ (Ruido Térmico Real)', color='#dd8452', alpha=0.8)
    ...
    plt.title('Evaluación de Resiliencia VQE (6 Qubits / 64 Estados)', fontsize=15, fontweight='bold', pad=15)
    plt.tight_layout()
    
    ruta_img = os.path.join(ruta_script, 'grafica_vqe_tfg.png')
