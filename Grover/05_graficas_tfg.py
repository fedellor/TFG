...
Descripción: Genera un gráfico de doble eje ordenado por rendimiento clásico.
"""
import json
import os
...
def generar_grafico_masivo(umbral=85.0):
    ...
    # ORDENAR LOS ESTADOS: De peor precisión clásica a mejor precisión
    ...
    c_ideal = datos.get("counts_ideal", {e: 0 for e in estados_ordenados})
    c_ruido = datos.get("counts_ruido", {e: 0 for e in estados_ordenados})
    ...
    ax1.bar(x - width/2, prob_ideal, width, label='Grover Ideal (Supera el umbral)', color='#4c72b0', alpha=0.9)
    ax1.bar(x + width/2, prob_ruido, width, label='Grover NISQ (Ruido térmico)', color='#dd8452', alpha=0.8)
    ...
    # Ocultar las etiquetas del eje X porque 64 son demasiadas, dejamos solo los ticks
    ax1.set_xticks(x)
    ...
    # Línea del umbral
    ax2.axhline(y=umbral, color='#c44e52', linestyle='--', linewidth=2, label=f'Umbral Oráculo ({umbral}%)')
    ...
    plt.title('Evaluación de Escalabilidad (6 Qubits / 64 Estados)', fontsize=15, fontweight='bold', pad=15)
    plt.tight_layout()
    
    ruta_img = os.path.join(ruta_script, 'grafica_masiva_tfg.png')
...
if __name__ == "__main__":
    generar_grafico_masivo(umbral=85.0)